...
You can stop after any stage; for inference-only work, skip to **stage 04** once **`weights/fruitfly_pose_yolo11m.pt`** exists.
...
| **02** | YOLO-pose layout, `data.yaml`, weights · config | [`dataset/`](../dataset/), [`weights/`](../weights/), [`configs/`](../configs/) | [`02_data.md`](02_data.md) |
...
| **05** | Bundled training metrics, curves, sample overlays | **`results/`** (+ Ultralytics checkpoints under **`runs/train/<run>/weights/`** when you train locally) | [`05_training_and_metrics.md`](05_training_and_metrics.md) |
