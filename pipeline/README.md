...
Run stages in order for a full reproducible path **from environment to published tracking bundles**. Shortcut: use the shipped **`weights/fruitfly_pose_yolo11m.pt`** and jump to **stage 04** for inference-only work.
...
| **04** Inference / tracking | [`stages/stage_04_inference_tracking/`](stages/stage_04_inference_tracking/README.md) | Pose + ByteTrack CSV: **`track_video.py`**; **`examples/load_and_infer.py`** (minimal predict smoke). |
